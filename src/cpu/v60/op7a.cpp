...
UINT32 opSEARCHUB(UINT8 bSearch)
{
	...
	for (i = 0; i < f7aLenOp1; i++)
	{
		appb = (MemRead8(f7aOp1 + i) == (UINT8)f7aOp2);
...
UINT32 opSEARCHUH(UINT8 bSearch)
{
	...
	for (i = 0; i < f7aLenOp1; i++)
	{
		appb = (MemRead16(f7aOp1 + i * 2) == (UINT16)f7aOp2);
		if ((bSearch && appb) || (!bSearch && !appb))
			break;
	}
...
UINT32 opEXTBFZ(void) /* TRUSTED */
{
	...
	F7BCREATEBITMASK(f7aLenOp1);

	modWriteValW = (f7aOp1 >> bamOffset) & f7aLenOp1;
...
UINT32 opEXTBFS(void) /* TRUSTED */
{
	...
	modWriteValW = (f7aOp1 >> bamOffset) & f7aLenOp1;
	if (modWriteValW & ((f7aLenOp1 + 1) >> 1))
		modWriteValW |= ~f7aLenOp1;
...
UINT32 opINSBFR(void)
{
	...
	F7CCREATEBITMASK(f7aLenOp1);

	f7aOp2 += bamOffset / 8;
	appw = MemRead32(f7aOp2);
	bamOffset &= 7;

	appw &= ~(f7aLenOp1 << bamOffset);
	appw |=  (f7aLenOp1 & f7aOp1) << bamOffset;

	MemWrite32(f7aOp2, appw);
...
//  { int a = 1; }
	...
	f7bBamOffset1 += f7aLenOp1 - 1;
	f7bBamOffset2 += f7aLenOp1 - 1;

	f7aOp1 += f7bBamOffset1 / 8;
	f7aOp2 += f7bBamOffset2 / 8;

	f7bBamOffset1 &= 7;
	f7bBamOffset2 &= 7;

	srcdata = MemRead8(f7aOp1);
	dstdata = MemRead8(f7aOp2);
...
UINT32 opADDDC(void)
{
	...
	src = (UINT8)(f7aOp1 >> 4) * 10 + (UINT8)(f7aOp1 & 0xF);
...
UINT32 opSUBDC(void)
{
	...
	src = (UINT32)(f7aOp1 >> 4) * 10 + (UINT32)(f7aOp1 & 0xF);
...
UINT32 opCVTDPZ(void)
{
	...
	apph = (UINT16)(((f7aOp1 >> 4) & 0xF) | ((f7aOp1 & 0xF) << 8));
	apph |= (f7aLenOp1);
	apph |= (f7aLenOp1 << 8);
...
UINT32 opCVTDZP(void)
{
	...
	if ((f7aOp1 & 0xF0) != (f7aLenOp1 & 0xF0) || ((f7aOp1 >> 8) & 0xF0) != (f7aLenOp1 & 0xF0))
	{
		// Decimal exception
		logerror("CVTD.ZP Decimal exception #1!\n");
	}

	if ((f7aOp1 & 0xF) > 9 || ((f7aOp1 >> 8) & 0xF) > 9)
	{
		// Decimal exception
		logerror("CVTD.ZP Decimal exception #2!\n");
	}

	appb = (UINT8)(((f7aOp1 >> 8) & 0xF) | ((f7aOp1 & 0xF) << 4));
	if (appb != 0) _Z = 0;
...
UINT32 (*const Op59Table[32])(void) =
{
	opADDDC,
	opSUBDC,
	opSUBRDC,
...
UINT32 (*const Op5BTable[32])(void) =
{
	opSCH0BSU,
	op5BUNHANDLED,
	opSCH1BSU,
...
UINT32 (*const Op5DTable[32])(void) =
{
	op5DUNHANDLED,
	op5DUNHANDLED,
	op5DUNHANDLED,
...
UINT32 (*const Op58Table[32])(void) =
{
	opCMPCB,
	opCMPCFB,
	opCMPCSB,
...
UINT32 (*const Op5ATable[32])(void) =
{
	opCMPCH,
	opCMPCFH,
	opCMPCSH,
